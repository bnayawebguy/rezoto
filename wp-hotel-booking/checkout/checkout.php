	...
	<div id="hotel-booking-payment">
		...
		<form name="hb-payment-form" id="hb-payment-form" method="post"
			...
			<table class="hb_table">
				<thead>
				<tr>
					<th class="hb_room_type"><?php _e( 'Room type', 'rezoto' ); ?></th>
					<th class="hb_capacity"><?php _e( 'Capacity', 'rezoto' ); ?></th>
					<th class="hb_quantity"><?php _e( 'Quantity', 'rezoto' ); ?></th>
					<th class="hb_check_in"><?php _e( 'Check - in', 'rezoto' ); ?></th>
					<th class="hb_check_out"><?php _e( 'Check - out', 'rezoto' ); ?></th>
					<th class="hb_night"><?php _e( 'Night', 'rezoto' ); ?></th>
					<th class="hb_gross_total"><?php _e( 'Gross Total', 'rezoto' ); ?></th>
				</tr>
				</thead>
				...
				<?php if ( $rooms = $cart->get_rooms() ) {
					foreach ( $rooms as $cart_id => $room ) {
						...
						<tr class="hb_checkout_item" data-cart-id="<?php echo esc_attr( $cart_id ); ?>">
							...
							<td class="hb_capacity"><?php echo sprintf( _n( '%d adult', '%d adults', $room->capacity, 'rezoto' ), $room->capacity ); ?> </td>
				...
				<tr class="hb_sub_total">
					<td colspan="7"><?php _e( 'Sub Total', 'rezoto' ); ?>
				...
				<?php if ( $tax = hb_get_tax_settings() ) { ?>
					<tr class="hb_advance_tax">
						<td colspan="7">
							<?php _e( 'Tax', 'rezoto' ); ?>
							<?php if ( $tax < 0 ) { ?>
								<span><?php printf( __( '(price including tax)', 'rezoto' ) ); ?></span>
							<?php } ?>
				...
				<?php if ( $advance_payment = $cart->advance_payment ) { ?>
					<tr class="hb_advance_payment">
						<td colspan="7">
							<?php printf( __( 'Advance Payment (%s%% of Grand Total)', 'rezoto' ), hb_get_advance_payment() ); ?>
						...
						<tr class="hb_payment_all">
							<td colspan="7" class="hb-align-right">
								<label class="hb-align-right check-wrap">
									...
									<p>
										<?php _e( 'I want to pay all', 'rezoto' ); ?>
									</p>
								</label>
							</td>
						</tr>
			...
			<?php if ( ! is_user_logged_in() && ! hb_settings()->get( 'guest_checkout' ) ) { ?>
				<?php printf( __( 'You have to <strong><a href="%1$s">login</a></strong> or <strong><a href="%2$s">register</a></strong> to checkout.', 'rezoto' ), wp_login_url( hb_get_checkout_url() ), wp_registration_url() ) ?>
			<?php } else { ?>
				...
				<?php if ( $tos_page_id = hb_get_page_id( 'terms' ) ) { ?>
					<p>
						<label>
							...
							<?php printf( __( 'I agree with ', 'rezoto' ) . '<a href="%s" target="_blank">%s</a>', get_permalink( $tos_page_id ), get_the_title( $tos_page_id ) ); ?>
						</label>
					</p>
				<?php } ?>
				<p>
					<button type="submit" class="hb_button"><?php _e( 'Check out', 'rezoto' ); ?></button>
				</p>
